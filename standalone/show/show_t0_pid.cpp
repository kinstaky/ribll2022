...
int main(int argc, char **argv) {
	...
	TH2F d1d2_pid(
		...
		2500, 0, calibration ? 250 : 50000,
		3000, 0, calibration ? 300 : 60000
	);
	...
	TH2F d2d3_pid(
		...
		1750, 0, calibration ? 175 : 35000,
		2000, 0, calibration ? 200 : 40000
	);
		...
		if (calibration) {
			for (int i = 0; i < t0_event.num; ++i) {
				for (int j = 0; j < 3; ++j) {
					t0_event.energy[i][j] =
						t0_param[j][0]
						+ t0_param[j][1] * t0_event.energy[i][j];
				}
			}
		}
