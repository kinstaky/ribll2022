...
int main(int argc, char **argv) {
	...
	// DSSD name
	std::string dssd_name = argv[pos_start+1];

	// pointer to DSSD
	std::shared_ptr<Dssd> dssd =
		CreateDssd(dssd_name, run, tag);
	if (!dssd) {
		std::cerr << "Error: Dssd " << dssd_name << " not found.\n";
		return -1;
	}

	if (dssd->CutBeamThreshold()) {
		std::cerr << "Error: Cut beam or under threshold events in "
			<< dssd_name << " failed.\n";
		return -1;
	}
	return 0;
