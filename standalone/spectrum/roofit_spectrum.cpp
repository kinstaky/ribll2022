...
int main() {
	...
	// gaussian at 13.9 MeV
	RooRealVar mean139("mean139", "mean", 13.65, 13.6, 13.7);
	RooRealVar sigma139("sigma139", "sigma", 0.15, 0.1, 0.2);
	RooGaussian gaus139("gaus139", "gaus", x, mean139, sigma139);
	...
	RooRealVar mean172("mean172", "mean", 17.2, 17.0, 17.8);
	...
	// summary
	RooGaussian* gaus_ex0[16] = {
		&gaus139,
		&gaus149, &gaus154, &gaus165, &gaus172, &gaus177,
		&gaus185, &gaus193, &gaus201, &gaus214,
		&gaus224, &gaus238, &gaus245, &gaus259,
		nullptr, nullptr
	};
	RooRealVar* frac0[16];
	RooAbsPdf* model0[16];
	...
	for (size_t i = 1; i < n0; ++i) {
		...
		model0[i] = new RooAddPdf(
			...
			RooArgList(*model0[i-1], *gaus_ex0[i]),
			*frac0[i-1]
		);
	}
	...
	// summary
	RooGaussian* gaus_ex1[16] = {
		&gaus172_1, &gaus177_1, &gaus185_1,&gaus193_1,
		&gaus201_1,
		&gaus214_1, &gaus224_1, &gaus235, &gaus245_1,
		&gaus259_1, nullptr, nullptr, nullptr,
	...
	for (size_t i = 1; i < n1; ++i) {
		...
		model1[i] = new RooAddPdf(
			...
			RooArgList(*model1[i-1], *gaus_ex1[i]),
			*frac1[i-1]
		);
	}
	...
	for (size_t i = 1; i < n2; ++i) {
		...
		model2[i] = new RooAddPdf(
			...
			RooArgList(*model2[i-1], *gaus_ex2[i]),
			*frac2[i-1]
		);
	}
