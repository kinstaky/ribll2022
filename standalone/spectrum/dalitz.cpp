...
int main() {
	// input file name
	TString input_file_name = TString::Format(
		"%s%sC14-10Be-4He-2H-v3-sim.root",
		kGenerateDataPath,
		kSpectrumDir
	);
	// input file
	...
	ipt->SetBranchAddress("excited_energy", &c_excitation);
	...
	opt.Branch("be10_state", &event.be10_state, "state/I");
	...
	for (long long entry = 0; entry < ipt->GetEntriesFast(); ++entry) {
		...
		double be10_ex = be_state >= 0 ? be_excited_energy[be_state] : 0.0;
		...
		double li_momentum = p_li.R();
		// excited 6Li total energy
		double li_energy = (he_kinetic + mass_4he) + (d_kinetic + mass_2h);
		// excited 6Li mass
		double li_mass = sqrt(
			pow(li_energy, 2.0) - pow(li_momentum, 2.0)
		);
		...
		double b_momentum = p_b.R();
		// excited 12B total energy
		double b_energy = (be_kinetic + be10_ex + mass_10be)
			+ (d_kinetic + mass_2h);
		// excited 12B mass
		double b_mass = sqrt(
			pow(b_energy, 2.0) - pow(b_momentum, 2.0)
		...
		event.be10_state = be_state;
		event.li6_ex = li_ex;
		event.b12_ex = b_ex;
		event.c14_ex = c_excitation;
